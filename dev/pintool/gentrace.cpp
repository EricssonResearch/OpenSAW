/*
    This file is a modified version of the gentrace.cpp from BAP, the
    Binary Analysis Platform.
    Modifications done by Ericsson, the original version was released under GPL v2.
...
static std::map<ADDRINT, int> basic_blocks;
static std::map<string, std::pair<ADDRINT, ADDRINT>> blacklist;
...
KNOB<string> KnobBBLOut(KNOB_MODE_WRITEONCE, "pintool",
                        "b", "visited_blocks.cov",
                        "A CSV-file with found blocks on the first line, and visited on the second.");
...
/*
 addr:         last address in basic block executed
*/
ADDRINT TraceFallThrough(ADDRINT addr)
{
    basic_blocks[addr] |= BBLExit::BOTH_BRANCHES;
    return addr;
}

/*
 addr:         last address in basic block executed
 branch_taken: true if the then branch was taken
*/
ADDRINT TracePostBranch(ADDRINT addr, BOOL branch_taken)
{
    basic_blocks[addr] |=
        branch_taken ? BBLExit::THEN_BRANCH : BBLExit::ELSE_BRANCH;
    return addr;
}

bool in_blacklist(ADDRINT addr) {
  for (const auto entry : blacklist) {
    if (addr >= entry.second.first && addr <= entry.second.second) {
      return true;
    }
  }
...
VOID InstrBlockCounting(BBL bbl)
{
    ...
    // Don't add basic blocks from blacklisted Images.
    if (in_blacklist(address)) return;

    IMG img = IMG_FindByAddress(address);

    // Avoid invalid images.
    if (!IMG_Valid(img)) {
        // TODO: Why can it be invalid?
        LOG("INVALID IMAGE\n");
        return;
    }

    // Normalize address by removing the low address of the
    // containing Image.
    address -= IMG_LowAddress(img);

    // The address counts as one of the found basic blocks.

    if (INS_IsBranch(ins) && INS_HasFallThrough(ins)) {
        basic_blocks[address] = BBLExit::CONDITIONAL;

        // Add the branch-block, including taken branch,
        // to the map of executed blocks.
        INS_InsertCall(ins, IPOINT_TAKEN_BRANCH, (AFUNPTR) TracePostBranch,
            IARG_ADDRINT, address, IARG_BRANCH_TAKEN, IARG_END);

        INS_InsertCall(ins, IPOINT_AFTER, (AFUNPTR) TracePostBranch,
            IARG_ADDRINT, address, IARG_BRANCH_TAKEN, IARG_END);
    } else {
        basic_blocks[address] = BBLExit::NON_CONDITIONAL;

        // Add the fallthrough-block to the map of executed blocks.
        INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR) TraceFallThrough,
            IARG_ADDRINT, address, IARG_END);
    }
}
...
VOID ModLoad(IMG img, VOID *v)
{
    ...
    // Blacklist everything except main executable
    if (!IMG_IsMainExecutable(img)) {
      blacklist[name] = std::make_pair(IMG_LowAddress(img), IMG_HighAddress(img));
    }
...
#define SEGV_SIGNAL 11
#define ABRT_SIGNAL 6
...
VOID ExceptionHandler(THREADID threadid, CONTEXT_CHANGE_REASON reason, const CONTEXT *from, CONTEXT *to, INT32 info, VOID *v) {
    ...
    */
    ...
    if (reason == CONTEXT_CHANGE_REASON_FATALSIGNAL) {
        ...
        if ((int) info == SEGV_SIGNAL) {
            LOG("SEGV\n");
            std::cerr << "Intercepted SEGV" << endl;
        } else if ((int) info == ABRT_SIGNAL) {
            LOG("ABRT\n");
            std::cerr << "Intercepted ABRT" << endl;
        }
        FlushBuffer(false, from, threadid, false);
        Cleanup();
        exit(1);
...
VOID Cleanup()
{
    ...
    {
      ofstream file (KnobBBLOut.Value().c_str());

      auto begin = basic_blocks.begin();
      auto end = basic_blocks.end();

      if (begin != end) {
        file << begin->first << ':' << begin->second;
        ++begin;
      }
...
int main(int argc, char *argv[])
{
    ...
    //PIN_UnblockSignal(SEGV_SIGNAL, 1);
    //PIN_InterceptSignal(SEGV_SIGNAL, SEGVHandler, 0);
